...
std::unique_ptr<cv::Mat> ImageConvolute8U(cv::Mat& src, const std::vector<std::vector<int>>& kernel, const double multiplier)//用到
{
...
std::vector<std::vector<int>> CalculateGaussianKernel(int size, double sigma) //gaussian filter
{
    std::vector<std::vector<int>> result(size, std::vector<int> (size, 0));
    int range = size / 2;
 ...
 void ApplyCanny(cv::Mat& src, int thresholdMax, int thresholdMin,IpVec &ipts)
{
    ...
    auto NonMaximaSuppressionImage = NonMaximaSuppression(*MagnitudeImage, *Gh, *Gv);
     TraceEdgesHysteresis(*NonMaximaSuppressionImage,thresholdMax, thresholdMin,ipts);
}
...
std::unique_ptr<cv::Mat> NonMaximaSuppression(cv::Mat& src, cv::Mat& Gh, cv::Mat& Gv) //用到
{
...
std::unique_ptr<cv::Mat> TraceEdgesHysteresis(cv::Mat& src, int thresholdMax, int thresholdMin,IpVec &ipts) //用到
{
