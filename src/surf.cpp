...
void Surf::getOrientation()
{
  ...
  //std::cout << "/*   ipt->orientation  */" <<  ipt->orientation << '\n';
}
    ...
    while(j < 12)
    {
      ...
      for (int k = i; k < i + 9; ++k)
      {
        for (int l = j; l < j + 9; ++l)
        {
          ...
          sample_x = fRound(x + ( k*scale*co));
          sample_y = fRound(y + ( l*scale*co));
      ...
      desc[count++] = dx*gauss_s2;
       //std::cout << "/*   desc[count++] */"<<  desc[count]<< '\n';
