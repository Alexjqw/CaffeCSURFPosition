...
void drawPoint(IplImage *img, Ipoint &ipt)
{
  ...
  s = 3;
  o = ipt.orientation;
  r1 = fRound(ipt.y);//foat convert int
  c1 = fRound(ipt.x);
...
//! Draw a double images point
void drawPoint_Line_jqw(CvMat* pair_p,Ipoint &ipt_L,Ipoint &ipt_R,int c_shift,int i)
{
  float s, o;
  int r_l, c_l,r_r,c_r;
  ...
  s = 3;
  o = ipt_L.orientation;
  r_l = fRound(ipt_L.y);//foat convert int
  c_l = fRound(ipt_L.x);

  r_r = fRound(ipt_R.y);//foat convert int
  c_r = fRound(ipt_R.x)+c_shift;


  cvCircle(pair_p, cvPoint(c_l,r_l), fRound(s), COLOURS[ipt_L.clusterIndex%NCOLOURS], -1);
  cvCircle(pair_p, cvPoint(c_l,r_l), fRound(s+1), COLOURS[ipt_L.clusterIndex+1%NCOLOURS], 2);

  cvCircle(pair_p, cvPoint(c_r,r_r), fRound(s), COLOURS[ipt_R.clusterIndex%NCOLOURS], -1);
  cvCircle(pair_p, cvPoint(c_r,r_r), fRound(s+1), COLOURS[ipt_R.clusterIndex+1%NCOLOURS], 2);

  if (i%2==0)
  {
    cvLine( pair_p, cvPoint(c_l,r_l),cvPoint(c_r,r_r),CV_RGB(0,255,0));
  }
  else
  {
    cvLine( pair_p, cvPoint(c_l,r_l),cvPoint(c_r,r_r),CV_RGB(0,0,255));
  }

  //cvShowImage( "匹配的点", pair_p );
...
{
//  IplImage* c_left=cvLoadImage("c_d_left.jpg",0);
//  IplImage* c_right=cvLoadImage("c_d_right.jpg",0);
  CvMat part_p;
  CvSize c_size = cvGetSize(img_l);
  CvMat* left_mat = cvCreateMat(c_size.height, c_size.width,CV_8UC3);

  cvConvert (img_l , left_mat );

  CvMat* right_mat = cvCreateMat(c_size.height, c_size.width,CV_8UC3);
  cvConvert (img_r, right_mat );
